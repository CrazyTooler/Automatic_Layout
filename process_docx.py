...
class Article:
    ...
    def set_count(self):
        ...
        for para in self.paragraphs:
            ...
            count += len(para)
            self.para_char.append(len(para)-numcount//2-1+2)#这里的加2为每行首行缩进
        self.count_paras_char = count-numcount//2-1+2*len(self.paragraphs)#这里的加2为每行首行缩进
    ...
    def set_default_size(self):
        ...
        pic_space = 0
        for pic in self.pic:
            pic_space += (width**2)*pic[1] // pic[0]
        self.pics_space = pic_space
        title_hight = 50 * pt + 40 * pt * (self.count_sub_title) + gap * (self.count_sub_title + 1)#mm
        title_hight = title_hight // 3.5
        self.title_space = title_hight * width
        line_nums = (self.count_paras_char // (width * 2.35 // char_width)) + 1  
        self.text_space = line_nums * width * line_hight // 3.5
        self.article_space = title_hight * width + line_nums * width * line_hight // 3.5 +pic_space
...
def get_pictures(word_path, result_path,imglist):
    ...
    for img_path in img_paths:#重命名
        ...
        os.rename(img_path,'./img/'+new_name+suffix+'.'+new)
        '''
    return result
...
if __name__ == '__main__':
    ...
    for i in range(len(s)):
        ...
        while s[i] > 0:
            ...
            j += 1
    ##########
    pic_size = get_pictures('./test.docx','./img_article',imglist)
    print(pic_size)
    print(len(result))
    for n in result:
        result[n].set_count()
